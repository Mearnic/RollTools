...
namespace RollTools
{
    ...
    {
        public List<Tag> queryList(long id)
        {
            List<Tag> tables = null;
            string querySql = "select id, poll_id, name, is_use from tag where poll_id = " + id + " and is_use = '1'";
            dBManager.Open();
        ...
        {
            List<Tag> tables = null;
            string querySql = "select id, poll_id, name, is_use from tag where poll_id = " + id;
            dBManager.Open();
            SQLiteDataReader reader = dBManager.ExecuteQuery(querySql);
            tables = new List<Tag>();
            while (reader.Read())
        ...
        {
            string querySql = "insert into tag values ( " + tag.Id + "," + tag.Poll_id + ",'" + tag.Name + "','" + tag.Is_use + "')";
            dBManager.Open();
        ...
        {
            string querySql = "update tag set name = '" + tag.Name + "', is_use = '" + tag.Is_use+ "' where id=" + tag.Id;
            dBManager.Open();
            dBManager.Execute(querySql);
            dBManager.Commit();
            dBManager.Close();
